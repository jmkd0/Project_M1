...
void init_weight(Weights* weights){
    for(int i = 0; i < param.number_feature; i++){
        for(int j = 0; j < param.number_cluster; j++){
            weights->bottom_up[i][j] = (1.5 - 1.0)/(param.vigilance + 0.2);
             weights->top_down[j][i] = param.learning_rate / (param.learning_rate - 1 + param.number_feature) - 0.1;
        }
    }
}

/* Initialize values of activations Bottom (X) and Top (Y) */
void activation_y(double* feature, Activation* activation){
    //for(int i = 0; i < param.number_feature; i++)
    //    activation->X[i] = feature[i];
    for(int i = 0; i < param.number_cluster; i++)
        activation->Y[i] = 0;
}
void activation_x(double* feature, Activation* activation){
    for(int i = 0; i < param.number_feature; i++)
        activation->X[i] = feature[i];
}
...
void learning(DataIris* data, Weights* weights, Activation* activation){
    ...
    for (int epoch = 0; epoch < param.epoch; epoch++){       //Perform epoch itteration
        for(int i = 0; i < size.lineIris; i++){              //For each flower
            activation_y(data[i].dataIris, activation); //Init activaton
            //display_input_layer(data[i].dataIris);
            //double norm_feature = norm(data[i].dataIris);  //Get the norm of the feature
            ...
            while(reset){  
                activation_x(data[i].dataIris, activation);     
                update_bottom_up_activation(weights, activation);//Update activation for Top (Y) layer                            //Test for reset 
                winner = find_winner(activation->Y);        //Get the winner index in Y activation
                if(activation->Y[winner] == -1){            //If all activation is -1 this feature cannot be clustered
                    data[i].predict_label = 0;
                    break;
                }
                update_top_down_activation(data[i].dataIris, weights, activation, winner); //Update activation for Bottom (X) layer
                ...
                if(vigilance >= param.vigilance){                   //If the similarity rate is above 0.4
                    reset = FALSE;                                      //Reset becomes false
                    data[i].predict_label = winner + 1;             //And the label of this feature is the index of Y winner in the top
                }else{
                    activation->Y[winner] = -1;                     //This class will not participate the compétition the next reset
                    display_activation_y(activation);
                }
            }
